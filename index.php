...
if (!empty($event_type)) {
    $event_type_escaped = $conn->real_escape_string($event_type);
    $sql .= " AND event_type = '$event_type_escaped'";
}

if (!empty($person)) {
    $person_escaped = $conn->real_escape_string($person);
    $sql .= " AND person = '$person_escaped'";
}
...
if ($result && $result->num_rows > 0) {
    ...
    // Дефолтные события
    $events = [
        ['id' => 1, 'title' => 'Запуск первого искусственного спутника Земли', 'description' => 'СССР запускает Спутник-1, открывая космическую эру человечества', 'event_date' => '1957-10-04', 'country' => 'СССР', 'event_type' => 'Запуски ракет', 'person' => '', 'image_url' => 'https://images.unsplash.com/photo-1614728263952-84ea256f9679?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=400'],
        ['id' => 2, 'title' => 'Первый полет человека в космос', 'description' => 'Юрий Гагарин совершает исторический полет на корабле "Восток-1"', 'event_date' => '1961-04-12', 'country' => 'СССР', 'event_type' => 'Пилотируемые полеты', 'person' => 'Ю. Гагарин', 'image_url' => 'https://images.unsplash.com/photo-1598431415913-48cc762b469b?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=400'],
        ['id' => 3, 'title' => 'Высадка на Луну', 'description' => 'Нил Армстронг и Базз Олдрин становятся первыми людьми на Луне', 'event_date' => '1969-07-20', 'country' => 'США', 'event_type' => 'Пилотируемые полеты', 'person' => 'Н. Армстронг', 'image_url' => 'https://images.unsplash.com/photo-1614726365930-627c75da663e?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=400']
    ];
}
